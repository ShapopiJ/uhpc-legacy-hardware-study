...
static_assert(sizeof(Cell) == 64, "Cell is not padded");

static Cell g_cell[2];

static_assert(alignof(g_cell) == 64, "cell array is not aligned");
...
static void help(FILE *f, const char *argv0)
{
    fprintf(f, "pingpong - measure inter thread notification overhead\n"
            ...
            "  -n COUNT          ping-pong iterations (default: 10^6)\n"
            "  -k COUNT          #iterations pause before storing (default: 1000)\n"
            ...
            "  --pause-tsc       like --spin-pause but limit spinning to 1 second\n"
            "  --umwait          spin less with UMONITOR/UMWAIT\n"
            "  --tsx             spin less with XBEGIN-style transactions\n"
            "  -p COUNT          #pauses after each atomic load,\n"
            "                    or pass 2 to enable c0.2 with --umwait or --tsx\n"
...
static int parse_args(Args *args, int argc, char **argv)
{
    *args = (const Args){0};
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (!strcmp(argv[i], "--pause-tsc")) {
            args->method = METHOD_PAUSE_TSC;
        } else if (!strcmp(argv[i], "--umwait")) {
#if __WAITPKG__
            args->method = METHOD_UMWAIT;
#else
            fprintf(stderr, "--umwait requires recompiling with `-march=native` or similar in CFLAGS1\n");
            exit(1);
#endif
        } else if (!strcmp(argv[i], "--tsx")) {
#if __RTM__
            args->method = METHOD_TSX;
#else
            fprintf(stderr, "--tsx requires recompiling with `-march=native -mrtm` or similar in CFLAGS1\n");
            exit(1);
#endif
...
static void *pause_tsc_main(void *p)
{
    Worker *x = (Worker*) p;
    Worker w = *x;
    ...
    for (unsigned i = 0; i < w.n; ++i) {
        if (i % 2 == w.init) { // sender
            ...
            end += 1000000000ul * 4; // wait up to ~ 1 s
            while (__rdtsc() < end) {
                new_tsc = atomic_load_explicit(&g_cell[w.init].tsc,
                        memory_order_consume);
                if (new_tsc > tsc) {
...
#if __WAITPKG__
static void *umwait_main(void *p)
{
    Worker *x = (Worker*) p;
    Worker w = *x;
    unsigned char c0 = 1; // == c01
    if (w.p == 2)
        c0 = 0; // == c02
    ...
    for (unsigned i = 0; i < w.n; ++i) {
        if (i % 2 == w.init) { // sender
            ...
            for (;;) {
                for (;;) {
                    unsigned tx = _xbegin();
                    if (tx != _XBEGIN_STARTED) {
                        // tx == 6 == (_XABORT_RETRY | _XABORT_CONFLICT)
                        //    => shared tsc was written after our explicit load
                        break;
                    }
                    new_tsc = atomic_load_explicit(&g_cell[w.init].tsc,
                            memory_order_consume);
                    // shared tsc is now part of the transaction's read-set
                    if (new_tsc > tsc) {
                        // unlikely, i.e. if we don't need to wait, at all
                        _xend();
                        break;
                    }
                    // if other thread writes shared tsc while tpause executes
                    // our transaction is rolled back
                    _tpause(c0, -1);
                    _xend();
                }
                new_tsc = atomic_load_explicit(&g_cell[w.init].tsc,
    ...
    for (unsigned i = 0; i < 2; ++i) {
        ...
        switch (args->method) {
            ...
            case METHOD_SPIN_PAUSE_MORE:
                ...
                break;
            case METHOD_PAUSE_TSC:
                r = pthread_create(&ws[i].worker_id, &attr, pause_tsc_main,
                        ws+i);
                break;
#if __WAITPKG__
            case METHOD_UMWAIT:
                r = pthread_create(&ws[i].worker_id, &attr, umwait_main,
                        ws+i);
                break;
#endif
#if __RTM__
            case METHOD_TSX:
                r = pthread_create(&ws[i].worker_id, &attr, tsx_main,
                        ws+i);
                break;
#endif
